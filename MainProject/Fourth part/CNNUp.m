...
% Train
XTrain = xt(:,:,:,indexTrain) ; % 4D (128, 130, 1, 5040)
YTrain = yt(indexTrain,1) ; % 1D (5040, 1)
PTrain = pt(indexTrain,1) ; % 1D (5040, 1)

% Test
XTest = xt(:,:,:,indexTest) ; % 4D (128, 130, 1, 560)
...
miniBatchSize  = ceil((numel(YTrain)/Iterations_Per_Epoch)/500) ;
